...
if __name__ == "__main__":
    ...
    print (f" Total error is : {calibrate.mean_error()}") # the error should be closer to zero for more accuracy  
    
    calibrate.storeCalibrationResult("Logitech_WebCam.hdf5") # HDF5 filename to store the camera configuration
    # This file contains can use in MATLAB or opencv for correcting distorted images
    
    undistort = RemoveDistortion(calibrate) # giving the calibration object if it is not give it will run calibration again
